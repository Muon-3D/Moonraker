...
ZONE_TAB = "\n".join([
    "# tzdb timezone descriptions, trimmed for the tests",
...
def tzdata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    for zone in ("Europe/London", "Europe/Berlin", "America/New_York",
...
class TestTzdata:
    def test_a_row_naming_several_countries_counts_for_each(self):
        assert clock.zones_for_country("IM") == ["Europe/London"]
        assert clock.zones_for_country("se") == ["Europe/Berlin"]
